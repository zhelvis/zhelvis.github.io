...
foreword: 'Do you want to create light and blazing fast sites, using your favorite web framework, with support for the latest
SEO, PWA recommendations and interface availability without writing a single line of server code? You are welcome!' 
---

Do you want to create light and blazing fast sites, using your favorite web framework, with support for the latest
SEO, PWA recommendations and interface availability without writing a single line of server code? You are welcome!
...
  But often the use of static site generator (SSG), which create pages from
  received data according to a predetermined pattern

## Architecture

![Architecture JAM](./JAM_stack.png)

Cloud services are used as a backend. All development comes down to designing templates for SSG, which hosted in git repository with the data together. 
The repository connects to the [continuous delivery service (CD)](https://aws.amazon.com/devops/continuous-delivery/), 
which tracks changes in the source code, starts the generator and places the resulting pages in 
...
We will store blog posts in markdown markup. Let's create a separate folder for records:
...
Restart server. Going to the graphQL browser, we will see that new data has become available to us. 
...
Now program in `gatsby-node.js`, how posts will be rendered. 
...
Our post will be available at http://localhost:8000/blog/first-post/ after restart
...
The result of the SSG is saved in `public` folder.
...
Then we connect the necessary repository.
...
During building, Netlify downloads the source code from Github, launches Gatsby, and deploy the result of its work in its own 
CDN with adding of random domain and valid certificate. In the settings you can change the domain to your own, as well 
...
that adds `netlify-cms-app` to the generator build.
